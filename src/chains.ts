...
export interface TokenInfo {
  ...
  name: string;
  decimals: number;
  /** AggregatorV3 USD price feed (typically 8 decimals) for this token. */
  feed: string;
}
...
export interface ChainInfo {
  ...
  /** Native gas symbol. */
  nativeSymbol: string;
  /** CoW Protocol REST API base for this chain (empty if CoW not used). */
  cowApi: string;
  explorerName: string;
  explorerBase: string;
  tokens: readonly TokenInfo[];
  /** Optional protocol addresses specific to this chain. */
  protocol?: {
    factory?: string;
    stackDeployer?: string;
    usdg?: string;
 ...
 */
const RH_TOKENS: readonly TokenInfo[] = [
