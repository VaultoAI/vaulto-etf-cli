...
export const COMMANDS: CommandSpec[] = [
  ...
  },
  {
    name: "balances",
    summary:
      "Wallet basket-token balances + allowances toward the vault. With --shares, shows amounts needed to mint and whether funded/approved.",
    needsSigner: false,
  ...
  },
  {
    name: "approve",
    summary:
      "Approve ERC-20 spending for mint (vault) or zap (zapper). Dry-run unless --confirm. Use --all for every basket token, --share-token for vault shares (zap redeem).",
    needsSigner: true,
    flags: [
  ...
  },
  {
    name: "mint",
    summary:
      "Deposit proportional basket tokens → mint shares (depositExactForShares). Dry-run unless --confirm. Use --approve to infinite-approve basket tokens first. RH single-asset: prefer zap mint.",
    needsSigner: true,
    flags: [
      {
        name: "--shares",
  ...
  },
  {
    name: "redeem",
    summary:
      "Sell/burn vault shares → receive pro-rata basket tokens. Dry-run unless --confirm. No approvals needed for direct redeem. RH single-asset USDG: prefer zap redeem.",
    needsSigner: true,
    flags: [
      {
        name: "--shares",
  ...
  },
  {
    name: "rebalance",
    summary:
      "Rebalance ops. Subcommand: status | start | settle | execute. cow vaults: start/settle (CoW). v4 vaults: execute (Uniswap V4). Writes need PRIVATE_KEY; dry-run unless --confirm.",
    needsSigner: true,
  ...
  },
  {
    name: "factory",
    summary:
      "VaultFactoryV3 ops on Robinhood Chain. Subcommand: list | whitelist | create. create WRITE (dry-run unless --confirm).",
    needsSigner: true,
  ...
  },
  {
    name: "zap",
    summary:
      "RHZapper single-asset USDG mint/redeem. Subcommand: preview-mint | preview-redeem | mint | redeem. mint/redeem WRITE (dry-run unless --confirm).",
    needsSigner: true,
    positional: {
      name: "subcommand",
      required: true,
  ...
  },
  {
    name: "deploy",
    summary:
      "Build/run a forge deploy script from VAULTO_V2_DIR (DavidVaulto/ETFs). Dry-run (prints command) unless --confirm.",
    needsSigner: true,
    flags: [
      {
        name: "--script",
        takesValue: true,
        description:
          "Forge script name or path (DeployV2Base | DeployV2BNB | DeployV2BNBStocks | DeployV2BNBMag7 | DeployV3RHMag7 | DeployV4HandlerRHMag7 | DeployRHZapperMag7 | DeployVaultFactoryV3 | …).",
  ...
  },
  {
    name: "keeper",
    summary:
      "Run the off-chain keeper from VAULTO_V2_DIR/keeper. CoW (Base) or RH V4 (rh:*) based on vault kind. Mode: check | tick | loop.",
    needsSigner: true,
    positional: {
      name: "mode",
      required: false,
