...
/** Rebalance architecture for a vault. */
export type VaultKind = "cow" | "v4";
...
export interface VaultInfo {
  ...
  vault: string;
  /**
   * CoW RebalanceController address. Empty string for V4-handler vaults
   * (the handler owns all rebalance state).
   */
  controller: string;
  handler: string;
  /** Optional RHZapper for single-asset USDG mint/redeem (RH only). */
  zapper?: string;
  kind: VaultKind;
  /** Forge deploy script (in the ETFs repo) that produced this vault. */
  deployScript: string;
  note?: string;
}
...
export function rebalanceTarget(v: VaultInfo): string {
