...
import { listFactoryDeployments, listFactoryWhitelist, sendCreateEtf, } from "../onchain.js";
 ...
 */
export async function factory(opts, sel, sub, flags) {
    const chainSlug = (sel.chain ?? "rh").toLowerCase();
    const chain = getChainBySlug(chainSlug);
    if (!chain.protocol?.factory) {
        throw new InputError(`No VaultFactoryV3 on chain ${chain.slug}. Factory is currently deployed on --chain rh only.`);
    }
...
function buildCreateParams(chainId, flags) {
    if (!flags.name)
        throw new InputError("Missing --name.");
    if (!flags.symbol)
        throw new InputError("Missing --symbol.");
    if (!flags.tokens)
        throw new InputError("Missing --tokens (comma-separated symbols or 0x addresses).");
    if (!flags.weights)
        throw new InputError("Missing --weights (comma-separated 1e18-scaled uints summing to 1e18).");
    ...
    if (tokenRefs.length < 2)
        throw new InputError("--tokens needs at least 2 assets.");
    ...
    const tokens = tokenRefs.map((ref) => {
        if (/^0x[0-9a-fA-F]{40}$/.test(ref))
            return ref;
        const t = findTokenBySymbol(chainId, ref);
        if (!t)
            throw new InputError(`Unknown token "${ref}" on chain ${chainId}.`);
        return t.address;
    });
    const weights = weightStrs.map((w) => {
        if (!/^\d+$/.test(w))
            throw new InputError(`Bad weight "${w}" — must be uint256 string.`);
        return BigInt(w);
    });
