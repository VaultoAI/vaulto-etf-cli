...
import { createVault as apiCreateVault, SUPPORTED_TOKENS, baseScanAddressUrl, } from "../client.js";
...
function parseAsset(spec) {
    ...
    if (!token) {
        throw new ConfigError(`Unknown or unsupported asset "${symbolRaw}". Allowed: ${SUPPORTED_TOKENS.map((t) => t.symbol).join(", ")}. ` +
            `Use --asset SYMBOL:BPS (e.g. --asset WETH:6000).`);
    }
    const weight = Number(bpsRaw);
    if (!Number.isInteger(weight) || weight <= 0) {
        throw new ConfigError(`Invalid weight for ${symbol}: "${bpsRaw}". Must be a positive integer in basis points (bps).`);
    }
...
function buildRequest(args) {
    ...
    if (!args.symbol?.trim())
        throw new ConfigError("--symbol is required");
    ...
    if (!args.asset || args.asset.length < 2) {
        throw new ConfigError("At least 2 assets required. Use repeated --asset SYMBOL:BPS (e.g. --asset WETH:6000 --asset USDC:4000).");
    }
    const parsed = args.asset.map(parseAsset);
    const sumBps = parsed.reduce((s, p) => s + p.asset.weight, 0);
    if (sumBps !== 10000) {
        throw new ConfigError(`Asset weights must sum to exactly 10000 bps (got ${sumBps}).`);
    }
 ...
 */
export async function createVaultCommand(opts, args) {
    const request = buildRequest(args);
    if (!args.confirm) {
        emit({
            dryRun: true,
            message: "Dry run — nothing sent. This deploys real contracts on Base and costs gas. " +
