...
const COW_MODE_SCRIPT = {
    check: "check",
    tick: "start",
    loop: "loop",
};
/** RH V4 keeper npm scripts. */
const RH_MODE_SCRIPT = {
    check: "rh:check",
    tick: "rh:start",
    loop: "rh:loop",
};
 ...
 */
export async function keeper(opts, sel, mode) {
    const dir = requireV2Dir();
    const keeperDir = join(dir, "keeper");
    if (!existsSync(keeperDir))
        throw new InputError(`No keeper/ in ETFs repo: ${keeperDir}`);
    ...
    if (v.kind === "v4") {
        ...
        if (!npmScript)
            throw new InputError(`Unknown keeper mode: ${m}. Use check | tick | loop.`);
        ...
        const pk = process.env.RH_KEEPER_PRIVATE_KEY ||
        ...
        if (res.error)
            throw Object.assign(new Error(`keeper failed: ${res.error.message}`), { code: "KEEPER_ERROR" });
        if (res.status !== 0) {
    ...
    if (chain.id !== 8453) {
        throw Object.assign(new Error(`CoW keeper is hard-wired to Base (chainId 8453). Vault ${v.slug} is on ${chain.slug} (${chain.id}). ` +
            `RH V4 vaults: vaulto-etf keeper --vault vMAG7-RH. BNB CoW needs a multi-chain-aware keeper fork.`), { code: "KEEPER_CHAIN_UNSUPPORTED" });
    }
    const env = {
        ...process.env,
