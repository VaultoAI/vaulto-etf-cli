...
import { getChainBySlug } from "../chains.js";
...
function resolveDeployChain(sel, defaultChain) {
    if (sel.chain)
        return getChainBySlug(sel.chain);
    if (sel.vault)
        return vaultChain(resolveVault(sel));
    if (defaultChain)
        return getChainBySlug(defaultChain);
 ...
 */
export async function deploy(opts, sel, flags) {
    ...
    if (!flags.confirm) {
        emit({
            ...
            note: "Re-run with --confirm to execute. Forge scripts read PRIVATE_KEY / DEPLOYER_PRIVATE_KEY / BNB_DEPLOYER_PK / RH_DEPLOYER_PK from env (never on the CLI). Without --broadcast this is a simulation.",
        }, opts);
        return;
    }
    const res = spawnSync("forge", args, { cwd: dir, stdio: "inherit", env: process.env });
    if (res.error)
        throw Object.assign(new Error(`forge failed: ${res.error.message}`), { code: "FORGE_ERROR" });
    if (res.status !== 0) {
