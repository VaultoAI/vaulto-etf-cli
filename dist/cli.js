...
function parseArgs(argv) {
    const positional = [];
    const flags = {};
    const valueFlags = new Set(["vault", "chain"]);
...
async function main() {
    ...
    const opts = { human: flags["human"] === true && flags["json"] !== true };
    const sel = { vault: str(flags["vault"]), chain: str(flags["chain"]) };
    if (flags["help"] === true) {
        printHelp();
        return;
    }
